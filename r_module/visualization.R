...
#### check distance with at least one POI ----
...
poi_dis <- osm_poi_swiss[
    points_in_box(
        coords = osm_poi@coords, 
        bbox = osmar::center_bbox(
            all_activities$lon[1], all_activities$lat[1], 400, 400
        )
    ), 
]
...
near_poi1 <- osm_poi_swiss[
    points_in_box(
        coords = osm_poi_swiss@coords, 
        bbox = osmar::center_bbox(
            8.518702, 47.381040, 400, 400
        )
    ), 
]

poi_dis1 <- osm_poi_swiss@coords[
    points_in_box(
        coords = osm_poi_swiss@coords, 
        bbox = osmar::center_bbox(
            8.518702, 47.381040, 400, 400
...
nearest_dis1 <- geosphere::distGeo(
    p1 = c(8.518702, 47.381040), 
    p2 = poi_dis1
)
...
nearest_poi <- function(coords, osmpoi_shp){
    # Input description ----
    # 
...
# bb <- osmar::center_bbox(8.518702, 47.381040, 100, 100)
# 
# # x <- osm_poi@coords[1:5, ]
# # y <- osmar::center_bbox(8.518702, 47.381040, 60, 60)
# 
# system.time(
#   near_me <- osm_poi[
#     points_in_box(
#       osm_poi@coords, 
#       osmar::center_bbox(8.540324, 47.377903, 100, 100)), 
#   ]
# )
...
source('2_code/r_module/ggplot_shapefile.R', chdir = TRUE) 
...
png('4_visualization/2.1 Spatial distribution of activities in Switzerland.png',
    width = 15, height = 9, units = 'cm', res = 600)
ggplot_shapefile(
    swiss_kanton, xylimits = c(5.9, 10.55, 45.78, 47.85), spacing = c(0.5, 0.5),
    lon_lat_attribute = list(
        lon_breaks = seq(6, 10, 1), lat_breaks = seq(46, 47.5, 0.5)) ) +
    geom_point(data = activities[which(activities$in_switzerland ==TRUE), ], 
               aes(x = lon, y = lat), size = 0.02, shape = 20, 
               color = "red", alpha = 0.01)
dev.off()

png('4_visualization/2.2 Spatial distribution of activities around the world.png', 
    width = 15, height = 9, units = 'cm', res = 600)
ggplot_shapefile(
...
sort(table(legs$labels), decreasing = TRUE)

# Number of activities = 1415539, in swiss = 1293544 (/1374670 = 0.91)
length(which(activities$in_switzerland) ) / dim(activities)[1]

# count of labeled activities 536453 (/1415539 = 0.38)
length(which(activities$purpose_new != 'unknown')) / dim(activities)[1]

# count and proportion of each category of activities
cbind(table(activities$purpose_new[
  which(activities$purpose_new != 'unknown')]),
  poportion = as.numeric(table(activities$purpose_new[
    which(activities$purpose_new != 'unknown')])
    /length(which(activities$purpose_new != 'unknown'))) )

# activity duration
c(length(which(activities$duration < 0.5*3600)),
  length(which(activities$duration >= 0.5*3600 & activities$duration < 2*3600)),
  length(which(activities$duration >= 2*3600 & activities$duration < 4*3600)),
  length(which(activities$duration >= 4*3600 & activities$duration < 8*3600)),
  length(which(activities$duration >= 8*3600 & activities$duration < 12*3600)),
  length(which(activities$duration >= 12*3600)) ) / dim(activities)[1]

# activity start time
c( length(which(activities$starttime >= 6 & activities$starttime < 11)),
   length(which(activities$starttime >= 11 & activities$starttime < 14)),
   length(which(activities$starttime >= 14 & activities$starttime < 17)),
   length(which(activities$starttime >= 17 & activities$starttime < 22)),
   length(which(activities$starttime >= 22 | activities$starttime < 6)) ) / 
    dim(activities)[1]

# activity start time
c( length(which(activities$endtime >= 6 & activities$endtime < 11)),
   length(which(activities$endtime >= 11 & activities$endtime < 14)),
   length(which(activities$endtime >= 14 & activities$endtime < 17)),
   length(which(activities$endtime >= 17 & activities$endtime < 22)),
   length(which(activities$endtime >= 22 | activities$endtime < 6)) ) / 
    dim(activities)[1]


table(participants$household_size, useNA = "ifany")/dim(participants)[1]
table(participants$income, useNA = "ifany")/dim(participants)[1]
table(participants$education, useNA = "ifany")/dim(participants)[1]

age = table(participants$age, useNA = "ifany")
c(sum(age[1:11]), sum(age[12:21]), sum(age[22:31]), sum(age[32:48]), 
  age[49]) / dim(participants)[1]

table(participants$language, useNA = "ifany")

table(participants$sex, useNA = "ifany")
table(participants$sex, useNA = "ifany") / dim(participants)[1]

table(participants$citizen_1, useNA = "ifany")

table(participants$work_type, useNA = "ifany") / dim(participants)[1]

table(participants$employment_1, useNA = "ifany") / dim(participants)[1]

table(participants$own_car, useNA = "ifany") / dim(participants)[1]
table(participants$own_motorbike, useNA = "ifany") / dim(participants)[1]
table(participants$own_bike_ebike, useNA = "ifany") / dim(participants)[1]
table(participants$has_pt_pass_halffare, useNA = "ifany") / dim(participants)[1]
table(participants$has_pt_pass_regional, useNA = "ifany")/ dim(participants)[1]
table(participants$has_pt_pass_ga, useNA = "ifany") / dim(participants)[1]




# Trip purpose classification results visulization ------------------------
