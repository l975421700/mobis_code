...
if(isTRUE(reimpute)){
    ...
    imputed_activity_incremental_output_path =
        '/data/mobis/data/enrichments/run_gao/imputed_activity_incremental_trial.RData'
...
#### encode activities not in Swiss as travel
allnew_activities[is.na(start_inkanton), 'imputed_purpose'] <- 'Travel'
...
for(i in 1:ite_num){
    ...
    }else if(((ite_num-1)*10000 + 1) <= dim(allnew_activities)[1]){
    ...
    for(j in 1:dim(ite_timeslot_activity_count)[1]){
        ...
        row_index = which(minutes_share_notravel$timeslot == 
                              ite_timeslot_activity_count[j, ]$by15)
        # minutes_share_notravel[row_index, ]
        column_index = which(minutes_share_colnames == 
                                 ite_timeslot_activity_count[j, ]$imputed_purpose)
        
        set(minutes_share_notravel, i = row_index, 
            j = column_index,
            minutes_share_notravel[row_index, ..column_index] + 
...
save(minutes_share_notravel,
     file = '/data/students/qigao/scratch/2_output_analysis/minutes_share_notravel.RData')
...
load('/data/students/qigao/scratch/2_output_analysis/minutes_share_notravel.RData')

################ calculate the #th of day and week
minutes_share_notravel$dayth <- ceiling(as.numeric(difftime(
    minutes_share_notravel$timeslot,
    mobis_start_date, units='days')))
minutes_share_notravel$weekth <- (minutes_share_notravel$dayth - 1) %/% 7 + 1


################ calculate the #th of hour_minute
minutes_share_notravel$hour_minute_th <- 
    lubridate::hour(minutes_share_notravel$timeslot) * 4 + 
    lubridate::minute(minutes_share_notravel$timeslot) / 15 + 1


################ aggregate activities for each week and hour_minute
weekly_minutes <- minutes_share_notravel[
    , .(Home_total = sum(Home), Work_total = sum(Work), 
        ...
        Travel_total = sum(Travel)
        ),
    by = c('weekth', 'hour_minute_th')
]
weekly_minutes$Total = apply(weekly_minutes[, 3:10], 1, sum)
# minutes_share_notravel$total_minute = apply(minutes_share_notravel[, 2:9], 1, sum)
# all(weekly_minutes$Total == minutes_share_notravel[
...
weekly_minutes$Travel_share = 
    weekly_minutes$Travel_total / weekly_minutes$Total
...
rgb_colors = rgb(
    ...
    alpha = 1 - weekly_minutes$Travel_share
)


################ plot the minutes share
png('visualization/01_minutes_share/1.1.0_minutes_share_of_activities_no_travel.png',
